...
/// My Orders — POST `order/fetchMyPurchasedOrders`, tabs by status (same as sold orders).
...
class _PurchasedOrderTabs extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      final n = ProductPurchasedController.tabStatuses.length;
      return Column(
        children: [
...
class _PurchasedThumbnail extends StatelessWidget {
  final PurchasedOrderModel order;

  const _PurchasedThumbnail({required this.order});
      ...
      child: Column(
        ...
        children: [
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _PurchasedThumbnail(order: order),
